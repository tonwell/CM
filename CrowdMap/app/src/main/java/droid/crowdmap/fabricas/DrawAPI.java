...
public class DrawAPI {
    ...
    public static PolygonOptions fillQuad( LatLng p, double scale, double signal) {
        ...
        Log.i("PONTO P", "iLat: " + iLat + ", iLng: " + iLng);
	...
	double nVal = val * trunc;
		...
		if ( nVal % nScale == 0 ) id = -nVal / trunc;
        else id = -( nVal + nScale - ( nVal %  nScale ) ) / trunc;
	}        
	return id ;
    }
    ...
    public static double rounding(double val, int dec){
        return val < 0 ? Math.ceil(val*Math.pow(10, dec))/Math.pow(10, dec)  : Math.floor(val*Math.pow(10, dec))/Math.pow(10, dec);
    }
}
